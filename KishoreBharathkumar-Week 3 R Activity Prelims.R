...
KishoreTitanic <-data.frame(Titanic)
...
head(KishoreTitanic, 2)

install.packages(pkgs = "dplyr")
library(package = "dplyr")

slice(.data = KishoreTitanic, c(1:2))
...
Sex <-KishoreTitanic[,2]
Sex

Age <-KishoreTitanic[,3]
Age
...
SexandAgeDataframe<- data.frame(Sex, Age)
...
freqabove25 <- filter(.data = KishoreTitanic, Freq > 25)
...
ggplot(KishoreTitanic, aes(Survived, Freq)) +
  geom_col()
